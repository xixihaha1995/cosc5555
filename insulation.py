...
# W*(m^-2)*(K^-1)
kAir = 0.024
# kClo will be highly dependent on NU(porous media)
kClo = 0.047

#outside clo hConv should be independent on cloth material
# W*(m^-2)*(K^-1)
hConv = 3
...
# for TEnv, how can smart clothing adopt to it:
# by rhoClo, or hConvClo?yy
#T1
#While(l1NORM> 1e-7)
#     T2'
#     T3
...
for TEnv in range(TEnvLow,TEnvHigh):
    ...
    for rhoClo in np.linspace(0, 1, 101):
        ...
        C1 = 0
        D1 = kAir / thickAir
        E1 = Metab- kAir / thickAir * (TBody) - (
                1 - rhoClo) * qRS
        p1 = np.array([[A1, B1, C1, D1, E1]])
        try:
            TCloOneRoots = fqs.quartic_roots(p1)
        except ZeroDivisionError:
            continue
        # print("TCloOne:" + str(TCloOneRoots[0][1].real)+", TEnv:" + str(TEnv))
        if (TCloOneRoots[0][1].real > 0 and TCloOneRoots[0][
            1].real > TEnv):
            TCloOne = TCloOneRoots[0][1].real
        else:
            continue
        TCloTwo = TCloOne - L1NORM
        while(L1NORM > convergence):
            qRClOne = epsilonClo*sigma*TCloOne**4
            qRClTwo = epsilonClo * sigma * TCloTwo ** 4

            A3 = (1-rhoClo)*sigma
            B3 = 0
            C3 = 0
            D3 = hConv
            E3 = -hConv*TCloTwo + kAir/thickAir*(TBody - TCloOne) + (1-rhoClo)*(qRS) - -qRClOne- qRClTwo
            p3 = np.array([[A3, B3, C3, D3, E3]])
            try:
                T3Roots = fqs.quartic_roots(p3)
            except ZeroDivisionError:
                continue
            if (T3Roots[0][1].real > 0):
                T3 = T3Roots[0][1].real
            else:
                continue
            T2 = thickClo / 2/kClo*(2*epsilonClo*sigma*(TCloOne-L1NORM/2)**4 - epsilonClo*sigma*(TBody**4+T3**4)) - kAir*thickClo/kClo/thickAir*(TBody - TCloOne)+TCloOne
            L1NORM = abs(T2-TCloTwo)
            TCloTwo = T2
